...
{
    *p++ = value; *p++ = value>>8; return p;
}
static INLINE char *lay4(char *p, unsigned long value)
{
    *p++ = value; *p++ = (value >>= 8); *p++ = (value >>= 8); *p++ = (value >>= 8); return p;
}
...
char *sqz_it(char *src, int typ, long cnt, int siz)
{
    ...
    case TMP_TAG: {
            ...
            if ((value = cnt) != 0)
            {
                if (cnt < 0) value = -cnt;  /* get abs(cnt) */
                if (value < 128)
                {
...
char *unsqz_it(char *src)
{
    ...
    case TMP_EOF: return(sqz.b8);     /* this is easy */
    case TMP_ASTNG:           /* these are strings */
    case TMP_BSTNG: {
            typ &= TMP_SIZE;
            switch (typ)
    ...
    case TMP_START: {
            ...
            {  /* do all the elements */
                ...
                case EXPR_VALUE: {
                        ...
                        if (texp->expr_code != EXPR_IDENT) continue;
 ...
 */
{
    ...
    if (!output_files[OUT_FN_ABS].fn_present) return; /* nuthin' to do if no ABS wanted */
    if (tmp_pool_size == 0)
    {
    ...
    if (tmp_fp == 0 )
    {
        ...
        if (!options->miser)
        {
            ...
            if (itm_ptr != (char *)0) tmp.t->tf_tag = *itm_ptr;    /* in case type is tag */
            dst.t = tmp.t+1;
            src.c = itm_ptr;
 ...
 */
{
    ...
    else
    {
        ...
        if (options->miser)
        {
            ...
            tmps += ALIGN(tmps);
#if defined(DEBUG_LINK)
            if ((long)tmp_ptr&3) printf("read_from_tmp: started at unaligned %08lX\n", (char *)tmp_ptr);
            if ((long)tmps&3) printf("read_from_tmp: ended unaligned at %08lX\n", (char *)tmps);
#endif
            tmp_next = (struct tmp_struct *)tmps;
            return(rtmp.tf_type);
        }
 ...
 */
{
    ...
    if ((outxabs_fp = abs_fp) == 0) return(1); /* no output required */
    ...
    while (1)
    {
        ...
        case TMP_TEST: {
                ...
                cmd_type= "TEST";
                if (tmp_ptr->tf_type == TMP_BOFF) cmd_type = "BOFF";
                else if (tmp_ptr->tf_type == TMP_OOR) cmd_type = "OOR";
                read_from_tmp();
        ...
        case TMP_EXPR: {
		    ...
		    unsigned char mea_buf[4];	/* for endian-agnostic output */
                    if ((lc = tmp_ptr->tf_length) == 0) lc=1; /* get the count */
		    ...
		    ubp = mea_buf;
                    ...
                    {
                    case 'j': {       /* long, high byte of low word first */
#if (0)
			    ...
			    break;
                    ...
                    case 'J': {       /* long, low byte of high word first */
#if (0)
                            unsigned char b0,b1,b2,b3; /* bytes in long */
                            b0 = token_value&0xFF; /* pickup individual bytes */
                            b1 = (token_value>>8)&0xFF;
			...
			i = 4;     /* 4 bytes */
			break;     /* take easy out */
                    case 'W': flip = 1;   /* short, high byte first */
                    case 'w': {
			    ...
			    ubp[flip ^ 1] = (token_value >> 8) & 0xFF;
                    ...
                    case 'I': flip = 1;   /* short, high byte first */
                    case 'i': {       /* short, low byte first */
                            i = 2;     /* 2 bytes */
			    ...
			    ubp[flip ^ 1] = (token_value >> 8) & 0xFF;
                    ...
                    case 'U': flip = 1;   /* short, high byte first */
                    case 'u': {       /* short, low byte first */
                            i = 2;     /* 2 bytes */
                            if (token_value & 0xFFFF0000) trunc_err(65535L);
			    ubp[flip] = (token_value) & 0xFF;
			    ubp[flip ^ 1] = (token_value >> 8) & 0xFF;
                            break;
                        }
                    case 'b': {       /* byte is the same as signed char */
                            i = 1;     /* char */
			    ...
			    *ubp = (token_value) & 0xFF;
                    ...
                    case 'c': {       /* char */
                            i = 1;
                            if (token_value & 0xFFFFFF00) trunc_err(255L);
			    *ubp = (token_value) & 0xFF;
                            break;
                        }
                    }             
        ...
        case TMP_BSTNG: 
        case TMP_ASTNG: {
                if (noout_flag == 0) outbstr((unsigned char *)tmp_pool,(int)tmp_ptr->tf_length);
