/*
    add_defs.c - Part of llf, a cross linker. Part of the macxx tool chain.
...
/*
 * At entry:
 *   retptr - pointer to place result
 */
{
    int err,i;
    err = sys$search(&fab);
...
#undef DOS_PATH
    ...
    tmp = lp = MEM_alloc(pathlen+2*namelen+extlen+3);
    if (lp == 0)
    {
    ...
    *tmp = 0;
    ...
    if ((typtr = strrchr(rp, '.')) != 0)
    {
        if (pathlen == 0)
        {
            MEM_free(tmp);     /* done with tmp area */
 ...
 * At entry:
 */
char   *src_nam,           /* source filename filename string */
char **inp_default_types,  /* default filetype string(s) */
char **default_paths,      /* default path string(s) */
int io,                    /* 0 = test for input,
                              1 = test for output,
                             -1 = parse syntax only */
FILE_name **retptr)        /* ptr to result is placed in here */
{
    int err;
    char *s;
    char **default_types;
    ...
    *retptr = fill_in_file();
    ...
    if (default_paths == 0)
    {
        if (our_cwd[0] == 0)
        {
            int cl;
            char *tmps;
            our_cwd[0] = (char *)MEM_alloc(256+2);
#if MALLOCDEBUG
